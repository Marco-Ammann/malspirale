...
export class AdminContentComponent implements OnInit {
  ...
  loadSelectedContent(): void {
    ...
    this.dataService.getContent(this.selectedPage).then(content => {
  ...
  deleteSection(index: number): void {
    if (confirm('Möchtest du diesen Abschnitt wirklich löschen?')) {
      ...
      if (this.currentSectionIndex === index) {
        this.currentSectionIndex = Math.min(index, this.contentSections.length - 1);
  ...
  moveSection(index: number, direction: number): void {
    ...
    [this.contentSections[newIndex], this.contentSections[index]];
  ...
  moveGalleryImage(index: number, direction: number): void {
    const galleryImages = this.contentSections[this.currentSectionIndex].galleryImages;
    ...
    [galleryImages[newIndex], galleryImages[index]];
  }
  ...
  async saveContent(): Promise<void> {
    ...
    try {
      ...
      this.saveMessage = 'Fehler beim Speichern des Inhalts. Bitte versuche es erneut.';
    } finally {
      this.saving = false;
